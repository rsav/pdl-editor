...
public class FunctionExpressionDialog extends JDialog {
	...
	private MapComboBoxModel comboBoxModelExpression;
	 ...
	 * Create the dialog.
	 */
	public FunctionExpressionDialog(TreeMap<String,Expression> me, MapComboBoxModel mce, JComboBox ce, TreeMap<String,PDLParameter> mp) {
		...
		lblExpression.setBounds(6, 34, 75, 16);
		contentPanel.add(lblExpression);
		
		
		comboBoxModelExpression = new MapComboBoxModel(mapExps);
		final JComboBox comboBoxExpression = new JComboBox(comboBoxModelExpression);
		comboBoxExpression.setBounds(79, 30, 134, 27);
		contentPanel.add(comboBoxExpression);
		...
		lblFunction.setBounds(6, 62, 61, 16);
		contentPanel.add(lblFunction);
		
		final JComboBox comboBoxFunction = new JComboBox(FunctionType.values());
		comboBoxFunction.setBounds(79, 58, 134, 27);
		contentPanel.add(comboBoxFunction);
		...
		okButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(newName.isEmpty()) {
					JOptionPane.showMessageDialog(getContentPane(),"No name entered. Cannot create FunctionExpression","Error",JOptionPane.ERROR_MESSAGE);
				} else {
					...
					if(nameUsed==false) {
						...
						// get the expression selected
						String expression = (String) comboBoxExpression.getSelectedItem();
						Expression newExpression = mapExps.get(expression);
						
						// get the function selected
						FunctionType newFunctionType = (FunctionType) comboBoxFunction.getSelectedItem();
						...
						Function newFunction = new Function()
														...
														.withFunctionName(newFunctionType);
						
						System.out.println("DEBUG FunctionExpressionDialog.okButton: Creating new FunctionExpression:");
						System.out.println("DEBUG FunctionExpressionDialog.okButton: expression="+newExpression);
						System.out.println("DEBUG FunctionExpressionDialog.okButton: function="+newFunction);
						System.out.println("DEBUG FunctionExpressionDialog.okButton: power="+newPowerExpression);
						System.out.println("DEBUG FunctionExpressionDialog.okButton: operation="+newOperation);
							
						
						// create the new expression of type FunctionExpression
						FunctionExpression newExp = new FunctionExpression()
						...
						System.out.println("DEBUG FunctionExpressionDialog.okButton: Adding new expression to mapExps");
